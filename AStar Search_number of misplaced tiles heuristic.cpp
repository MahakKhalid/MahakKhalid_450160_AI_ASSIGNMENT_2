...
struct compare {
    bool operator()(const state &v, const state &b) const {
        int gm = manhattan(v.first) + v.second;  // g(n) + h(n)
        int hm = manhattan(b.first) + b.second;  // g(n) + h(n)
        return gm > hm;
    }
};
...
void a_star_search(const vector<vector<int>> &initial_state) {
    ...
    while (!que.empty()) {
        ...
        int current_cost = que.top().second;
        que.pop();
        ...
        for (auto &neighbor : neighbors) {
            ...
            if (!visited[neighbor] || new_cost < cost[neighbor]) {
                cost[neighbor] = new_cost;
                parent[neighbor] = current_state;
                que.push({neighbor, new_cost});
            }
        }
    }
}
...
int main() {
    ...
    cout << "\n\nImplementing A* Search using Manhattan distance heuristic\n\n";
