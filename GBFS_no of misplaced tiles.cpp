...
int misplaced_tiles(const vector<vector<int>> &c) {
    int misplaced = 0;
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (c[i][j] != 0 && c[i][j] != goal_state[i][j]) {
                misplaced++;
            }
        }
    }
...
struct compare {
    bool operator()(const state &v, const state &b) const {
        return misplaced_tiles(v.first) > misplaced_tiles(b.first);
    }
};
...
int main() {
    ...
    cout << "\n\nImplementing Greedy Best-First Search using Misplaced Tiles heuristic\n\n";
